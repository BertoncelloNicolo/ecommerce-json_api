...
addRoute('POST', '/products', function () {
    ...
    if ($jsonData && isset($jsonData['data']['attributes'])) {
        ...
        if ($newProduct) {
            $responseData = ['data' => ['type' => 'products', 'id' => $newProduct->getId(), 'attributes' => ['nome' => $newProduct->getNome(), 'marca' => $newProduct->getMarca(), 'prezzo' => $newProduct->getPrezzo()]]];
...
addRoute('PATCH', '/products/(\d+)', function ($matches) {
    $parts = explode('/', $matches); // sostituizione split con explode perche con soluzione non funzionava
    $id = end($parts); // andiamo a prendere l'id con la funzione end che prendere l'ultimo elemento dell'array parts
    $patchData = json_decode(file_get_contents("php://input"), true);
    $product = Product::Find($id); //andiamo a cercare il prodotto con il metodo find della classe products
    try {
